...
API for accessing and building items inside a New Quiz. To interact with the quiz itself, see the [New Quizzes API](https://developerdocs.instructure.com/services/canvas/resources/new_quizzes).
...
2. `PUT <media_upload_url>` with the image included in the request as a data attribute. For example, in cURL:
   ...
   ```
3. `POST /api/quiz/v1/courses/:course_id/quizzes/:assignment_id/items`, including the *unsigned* media\_upload\_url in the request's interaction\_data param, like this:
