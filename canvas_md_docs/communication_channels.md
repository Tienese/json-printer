...
| `communication_channel[type]`    | Required `string` | <p>The type of communication channel.</p><p><br></p><p>In order to enable push notification support, the server must be properly configured (via ‘sns\_creds<code>in Vault) to communicate with Amazon Simple Notification Services, and the developer key used to create the access token from this request must have an SNS ARN configured on it.\</p> Allowed values:</code>email<code>,</code> sms<code>,</code> push\`</p> |
