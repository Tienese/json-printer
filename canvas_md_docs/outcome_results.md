...
| `exclude[]`           | `string`  | <p>Specify additional values to exclude. “missing\_user\_rollups” excludes rollups for users without results.</p><p>Allowed values: <code>missing\_user\_rollups</code></p>                                                                                                                                                                            |
| `sort_by`             | `string`  | <p>If specified, sorts outcome result rollups. “student” sorting will sort by a user’s sortable name. “outcome” sorting will sort by the given outcome’s rollup score. The latter requires specifying the “sort\_outcome\_id” parameter. By default, the sort order is ascending.</p><p>Allowed values: <code>student</code>, <code>outcome</code></p> |
...
### [Enqueue a delayed Outcome Rollup Calculation Job](#method.outcome_results.enqueue_outcome_rollup_calculation) <a href="#method.outcome_results.enqueue_outcome_rollup_calculation" id="method.outcome_results.enqueue_outcome_rollup_calculation"></a>

[OutcomeResultsController#enqueue\_outcome\_rollup\_calculation](https://github.com/instructure/canvas-lms/blob/master/app/controllers/outcome_results_controller.rb)
